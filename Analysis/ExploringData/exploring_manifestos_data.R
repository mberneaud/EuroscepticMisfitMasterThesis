...
mp_setapikey("./Analysis/ExploringData/manifesto_apikey.txt")
data <- mp_maindataset()

# Selected only EU Member States and saved them to a new .csv in Data folder
EUMS <- filter(data, eumember == 10)
write.csv(EUMS, file = "./Analysis/Data/CMP_EUMS.csv")
dateDownloaded <- date()
write(dateDownloaded, file = "./Analysis/Data/CMP_EUMS_download_date")
