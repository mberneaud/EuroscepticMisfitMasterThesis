...
drawgg <- function(cid ,cname) {
  tmp <- dplyr::filter(CYmerge, nation == cid)
  tmp <- select(tmp, nation, year, sum.EUS.vote, avg.EUS, diff.EUS)
  g <- ggplot(tmp, aes(x = year)) +
    geom_line(aes(y = avg.EUS, colour = "avg.EUS")) +
    geom_line(aes(y = sum.EUS.vote, colour = "sum.EUS.vote")) +
    geom_line(aes(y = diff.EUS, colour = "diff.EUS")) +
    ...
    theme(legend.position = "top") + ggtitle(cname)
  class(g) <- "ggplot"
  print(g)
}
